...
namespace WindowsTerminalQuake.TerminalBoundsProviders
{
	public class GenericProcessProvider : IProcessProvider
	{
		...
		private Process GetOrCreate(string[] args)
		{
			...
			//QSettings.Instance.WindowsTerminalCommand = @"C:\Downloads\WezTerm-windows-20210502-154244-3f7122cb\wezterm-gui.exe";
			...
			if (process == null)
			{
				...
				{
					throw new Exception($"Could not find the process exe at '{QSettings.Instance.WindowsTerminalCommand}'. Make sure it is installed, and see '{nameof(QSettings.Instance.WindowsTerminalCommand)}' setting for more information.");
				}
