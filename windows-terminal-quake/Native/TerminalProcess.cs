...
namespace WindowsTerminalQuake.Native
{
	public static class TerminalProcess
	{
		...
		private static Process GetOrCreate()
		{
			...
			Log.Information($"Got process with id '{process.Id}' and name '{process.ProcessName}' and title '{process.MainWindowTitle}'.");
			...
			if (process.MainWindowTitle == "DesktopWindowXamlSource") throw new Exception($"Process still has temporary 'DesktopWindowXamlSource' window title.");
