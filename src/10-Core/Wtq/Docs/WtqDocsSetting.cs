...
{
	private readonly DisplayAttribute? _displayAttr = prop.GetCustomAttribute<DisplayAttribute>();
	private readonly DisplayFlagsAttribute? _dispFlagsAttr = prop.GetCustomAttribute<DisplayFlagsAttribute>();
	private readonly JsonPropertyOrderAttribute? _orderAttr = prop.GetCustomAttribute<JsonPropertyOrderAttribute>();
	private readonly DefaultValueAttribute? _defaultValAttr = prop.GetCustomAttribute<DefaultValueAttribute>();
	private readonly ExampleValueAttribute? _exampleValAttr = prop.GetCustomAttribute<ExampleValueAttribute>();
	private readonly RequiredAttribute? _reqAttr = prop.GetCustomAttribute<RequiredAttribute>();
...
//		IsEnum
//			? string.Join(" | ", EnumValues?.Select(e => e.Value) ?? [])
//			: _exampleValAttr?.Value ?? DefaultValue;
